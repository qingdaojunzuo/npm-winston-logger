(function(root) {
  ...
  var consoleOld = console.log,
      stdoutOld = process.stdout.write,
      stderrOld = process.stderr.write;

  var overrideConsole = function(logger) {
    var log = function(level, a) {
      try {
        logger[level](util.convert(a));
      } catch (e) {
        consoleOld.call(console, util.convert(a));
      }
    };
    // override console functions
    console.log = function() {
      if (arguments.length === 1 && 
          arguments[0] === 'LISTENING')
        consoleOrig.call(console, 'LISTENING');
      else
